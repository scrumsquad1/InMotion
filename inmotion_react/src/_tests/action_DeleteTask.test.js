...
import {TYPE_TASKS_DELETETASK_COMPLETE} from '../_redux/actions/tasks/action_DeleteTask';
...
it('action_InsertTask works as expected', () => {
    ...
    store.dispatch({type: TYPE_TASKS_DELETETASK_COMPLETE, payload: newTask});
    expect(store.getState().listsStore.lists[newList.id].tasks.indexOf(newTask) > -1).toBe(false);
